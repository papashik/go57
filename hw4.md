...
Напишите функцию `BinSearch(arr []int, x int) bool` - функция принимает (указатель на) срез `arr`, отсортированный по возрастанию, и целое число `x` и должна возвращать `true` или `false` в зависимости от того, есть ли элемент `x` в массиве. 
